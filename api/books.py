...
from sqlalchemy.orm import Session # Додано імпорт
...
# Видалили глобальний book_service = BookService()
...
async def get_all_books(
    ...
    db: Session = Depends(get_db)  # Додали сесію БД
):
    """Отримання всіх книг з можливою фільтрацією та сортуванням."""
    book_service = BookService(db) # Створюємо сервіс тут
    return await book_service.get_books(status=status_filter, author=author, sort_by=sort_by)

@router.get("/{book_id}", response_model=BookResponse, status_code=status.HTTP_200_OK)
async def get_book_by_id(book_id: UUID, db: Session = Depends(get_db)): # Додали сесію БД
...
async def add_book(book: BookCreate, db: Session = Depends(get_db)): # Додали сесію БД
    """Додавання нової книги."""
    book_service = BookService(db)
    return await book_service.create_book(book)

@router.delete("/{book_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_book(book_id: UUID, db: Session = Depends(get_db)): # Додали сесію БД
